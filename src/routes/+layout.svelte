  ...
  >
	...
	{#if signUp && !phoneSignUp}
	  ...
	  <form class="flex flex-col gap-2 w-full justify-center" on:submit|preventDefault={handleSignup}>
		<input type="text" placeholder="Name" bind:value={name} class="rounded-lg text-[14px]" required />
		<input type="email" placeholder="Email" bind:value={email} class="rounded-lg text-[14px]" required />
		...
		<div class="flex items-start pt-1 pb-1">
		  ...
		  <label for="terms" class="ml-2 text-xs text-left">I acknowledge that this is a learning model and information provided is for educational purpose only.</label>
		</div>
		<button type="submit" class="w-full rounded-lg bg-[#294254] p-2 text-white text-center disabled:opacity-50" disabled={!termsAccepted}>Signup with Email</button>
	  </form>
	{:else if !signUp && !phoneSignUp}
	  ...
	  <form class="flex flex-col gap-2 w-full justify-center" on:submit|preventDefault={handleSignIn}>
		...
		<button type="submit" class="w-full rounded-lg bg-[#294254] p-2 text-white text-center disabled:opacity-50" disabled={!termsAccepted}>SignIn with Email</button>
	  </form>
	{:else}
	  {#if showNameBox}
		...
		<form class="flex flex-col gap-2 w-full justify-center" on:submit|preventDefault={setName}>
		  <input type="text" id="name" placeholder="Enter Name" bind:value={name} class="rounded-lg text-[14px]" required />
		  <button type="submit" id="sendOtp" class="w-full rounded-lg bg-[#294254] p-2 text-white text-center disabled:bg-[#111d25]">Continue</button>
		</form>
	  {:else}
		<p class="font-semibold mb-6">SignIn to your account!</p>
		<form class="flex flex-col gap-2 w-full justify-center" on:submit|preventDefault={handlePhoneSignIn}>
		  <input type="text" id="phone" placeholder="Enter Phone Number" bind:value={phone} class="rounded-lg text-[14px]" required />
		  <div id="recaptcha"></div>
		  {#if showOtp}
			<input type="text" placeholder="Enter OTP" bind:value={otp} class="rounded-lg text-[14px]" required />
			<button type="button" on:click={handleOtpVerification} class="w-full rounded-lg bg-[#294254] p-2 text-white text-center disabled:opacity-50" disabled={!termsAccepted}>Verify OTP</button>
		  {:else}
			<div class="flex items-start pt-1 pb-1">
			  <input type="checkbox" id="terms" bind:checked={termsAccepted} required />
			  <label for="terms" class="ml-2 text-xs text-left">I acknowledge that this is a learning model and information provided is for educational purpose only.</label>
			</div>
			<button type="submit" id="sendOtp" class="w-full rounded-lg bg-[#294254] p-2 text-white text-center disabled:opacity-50" disabled={!termsAccepted}>Send OTP</button>
		  {/if}
		</form>
	  {/if}
	{/if}
	<div class="link">
	  <p class="mb-2 -mt-2 font-medium">SignUp Using <button class="text-blue-600" on:click={() => { phoneSignUp = !phoneSignUp; }}>Phone ?</button></p>
	</div>
	<p class="font-bold -pt-2">Or</p>
	<button on:click={googleSignIn} class="w-full select-none flex flex-row justify-center gap-2 rounded-lg bg-black text-white shadow-gray-900/10 hover:shadow-gray-900/20 py-2 px-6 text-center items-center align-middle font-sans text-md font-bold shadow-md transition-all hover:shadow-lg focus:opacity-[0.85] focus:shadow-none active:opacity-[0.85] active:shadow-none disabled:pointer-events-none disabled:opacity-50 disabled:shadow-none" disabled={!termsAccepted}>
	  <img class="w-6 h-6" src="https://www.svgrepo.com/show/475656/google-color.svg" loading="lazy" alt="google logo" />
	  <span>Sign in with Google</span>
	</button>
	<div class="link">
	  <p class="mb-2 -mt-2 font-medium">Already have an account? <button class="text-blue-600" on:click={() => { signUp = !signUp; phoneSignUp = false; }}>Log In</button></p>
	</div>
  </Modal>
